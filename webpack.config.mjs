...
import { utillie } from './index.mjs';
import NodePolyfillPlugin from 'node-polyfill-webpack-plugin';
import webpack from 'webpack';

const { __dirname } = utillie.__(import.meta.url);
...
export default {
    ...
    ignoreWarnings: [warning =>
        ...
        ||
        (warning?.loc?.start?.line === 703 && warning?.loc?.start?.column === 44 && warning?.loc?.end?.line === 703 && warning?.loc?.end?.column === 56) // utillie 
    ],
    // stats: 'detailed',
};